...
void example_ks()
{
    ...
    */
    ...
    parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, { 60, 51, 51, 51, 60 }));
    ...
    */
    ...
    ofstream sk1;
    sk1.open("D:/SEAL/sk.txt", ios::binary);
    for (uint64_t i = 0; i < poly_modulus_degree * 5; i++)
    {
        sk1 << secret_key.data()[i] << endl;
    }
    sk1.close();

    ofstream pk1;
    pk1.open("D:/SEAL/pk.txt", ios::binary);
    for (uint64_t i = 0; i < poly_modulus_degree * 10; i++)
    {
        pk1 << public_key.data()[i] << endl;
    }
    pk1.close();
    ...
    */
    ...
    std::cout << "Encryption time = " << chrono::duration<double, milli>(diff).count() << " ms" << endl;
    cout << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$:" << endl;
    cout << "    + Scale of x^2 before rescale: " << log2(x3_encrypted.scale()) << " bits" << endl;

    cout << "    + Modulus chain index for x3_encrypted_1: "
         << context.get_context_data(x3_encrypted.parms_id())->chain_index() << endl;

    context_data = context.get_context_data(x3_encrypted.parms_id());
    cout << "Coeff_modulus primes_44444: ";
    cout << hex;
    for (const auto &prime : context_data->parms().coeff_modulus())
    {
        cout << prime.value() << " ";
    ...
    */
    ...
    cout << "    + Modulus chain index for x3_encrypted_2: "
         << context.get_context_data(x3_encrypted.parms_id())->chain_index() << endl;

    context_data = context.get_context_data(x3_encrypted.parms_id());
    cout << "Coeff_modulus primes_33333: ";
    cout << hex;
    for (const auto &prime : context_data->parms().coeff_modulus())
    {
        cout << prime.value() << " ";
    ...
    cout << "    + Modulus chain index for x3_encrypted_3: "
         << context.get_context_data(x3_encrypted.parms_id())->chain_index() << endl;

    context_data = context.get_context_data(x3_encrypted.parms_id());
    cout << "Coeff_modulus primes_22222: ";
    cout << hex;
    for (const auto &prime : context_data->parms().coeff_modulus())
    {
        cout << prime.value() << " ";
    ...
    */
    ...
    print_line(__LINE__);
    cout << "Compute, relinearize, and rescale (PI*x)*x." << endl;
    evaluator.multiply_inplace(x1_encrypted_coeff3, x1_encrypted);
    evaluator.relinearize_inplace(x1_encrypted_coeff3, relin_keys);
    cout << "    + Scale of (PI*x)*x before rescale: " << log2(x1_encrypted_coeff3.scale()) << " bits" << endl;
    evaluator.rescale_to_next_inplace(x1_encrypted_coeff3);
    cout << "    + Scale of (PI*x)*x after rescale: " << log2(x1_encrypted_coeff3.scale()) << " bits" << endl;
    ...
    */
    ...
    print_line(__LINE__);
    cout << "Compute, relinearize, and rescale (PI*x^2)*x^2." << endl;
    evaluator.multiply_inplace(x3_encrypted, x1_encrypted_coeff3);
    evaluator.relinearize_inplace(x3_encrypted, relin_keys);
    cout << "    + Scale of x^3 before rescale: " << log2(x3_encrypted.scale()) << " bits" << endl;
    evaluator.rescale_to_next_inplace(x3_encrypted);
    cout << "    + Scale of x^3 after rescale: " << log2(x3_encrypted.scale()) << " bits" << endl;

    cout << "    + Modulus chain index for x3_encrypted_4: "
         << context.get_context_data(x3_encrypted.parms_id())->chain_index() << endl;

    context_data = context.get_context_data(x3_encrypted.parms_id());
    cout << "Coeff_modulus primes_11111: ";
    cout << hex;
    for (const auto &prime : context_data->parms().coeff_modulus())
    {
        cout << prime.value() << " ";
    ...
    */
    ...
    cout << "    + Modulus chain index for x1_encrypted_coeff1: "
         << context.get_context_data(x1_encrypted_coeff1.parms_id())->chain_index() << endl;
    ...
    */
    ...
    cout << "    + Exact scale in  x^4: " << x3_encrypted.scale() << endl;
    cout << "    + Exact scale in  x: " << x1_encrypted_coeff1.scale() << endl;
    ...
    */
    ...
    x3_encrypted.scale() = pow(2.0, 51);
    x1_encrypted_coeff1.scale() = pow(2.0, 51);
    ...
    */
    print_line(__LINE__);
    cout << "Compute PI*x^4 + 0.4*x + 1." << endl;
    Ciphertext encrypted_result;
    evaluator.add(x3_encrypted, x1_encrypted_coeff1, encrypted_result);
    evaluator.add_plain_inplace(encrypted_result, plain_coeff0);

    context_data = context.get_context_data(x3_encrypted.parms_id());
    cout << "Coeff_modulus primes_00000: ";
    cout << hex;
    for (const auto &prime : context_data->parms().coeff_modulus())
    {
        cout << prime.value() << " ";
    ...
    */
    ...
    {
        double x = input[i];
        true_result.push_back((3.14159265 * x * x * x + 0.4) * x + 1);
    }
